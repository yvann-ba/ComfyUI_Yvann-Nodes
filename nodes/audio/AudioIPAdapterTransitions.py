    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "images": ("IMAGE", {
                    "forceInput": True,
                    "tooltip": "Batch of images for transitions (loops if fewer than peaks)"
                }),
                "peaks_weights": ("FLOATS", {
                    "forceInput": True,
                    "tooltip": "Peak weights from Audio Peaks Detection node"
                }),
                "transition_mode": (["linear", "ease_in_out", "ease_in", "ease_out"], {
                    "default": "linear",
                    "tooltip": "Blending curve between images"
                }),
                "transition_length": ("INT", {
                    "default": 5, "min": 1, "max": 100, "step": 2,
                    "tooltip": "Number of frames for each transition blend"
                }),
                "min_IPA_weight": ("FLOAT", {
                    "default": 0.0, "min": 0.0, "max": 1.99, "step": 0.01,
                    "tooltip": "Minimum IPAdapter weight per frame"
                }),
                "max_IPA_weight": ("FLOAT", {
                    "default": 1.0, "min": 0.1, "max": 2.0, "step": 0.01,
                    "tooltip": "Maximum IPAdapter weight per frame"
                }),
            }
        }
