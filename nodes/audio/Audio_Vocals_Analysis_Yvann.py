...
class Audio_Vocals_Analysis_Yvann(AudioNodeBase):
    ...
    RETURN_NAMES = ("Vocals Audio", "Vocals Weights", "Vocals Masks", "Weights Graph")
    ...
    def _rms_energy(self, waveform, num_frames, samples_per_frame):
        try:
            ...
            print(f"Error in RMS energy(Vocals Analysis algorithm) calculation: {e}")
    ...
    def process_audio(self, audio, video_frames, smoothing_factor, global_intensity):
        ...
        try:
            ...
            vocals_waveform = estimates[:, 0, :, :]
        except Exception as e:
            print(f"Error in model processing: {e}")
            return None, None, None, None

        vocals_audio = {
            'waveform': vocals_waveform.cpu(),
            'sample_rate': sample_rate,
        }
        
        vocals_weights = self._rms_energy(vocals_waveform.squeeze(0), num_frames, samples_per_frame)
        if np.isnan(vocals_weights).any() or np.isinf(vocals_weights).any():
            print("Invalid vocals weights calculated")
            return None, None, None, None

        vocals_weights = self._smooth_weights(vocals_weights, max(0.01, smoothing_factor))
        vocals_weights = self._normalize_weights(vocals_weights)
        if len(vocals_weights) != num_frames:
            print("Mismatch between vocals weights and video frames")
            return None, None, None, None

        vocals_weights = self.adjust_weights(np.array(vocals_weights), global_intensity)

        try:
            plt.figure(figsize=(10, 6))
            plt.plot(list(range(1, num_frames + 1)), vocals_weights, label='Vocals Weights', color='green')
            plt.xlabel('Frame Number')
            plt.ylabel('Normalized Weights')
            plt.title('Normalized Weights for Vocals (RMS Energy)')
            plt.legend()
            plt.grid(True)
        ...
        vocals_masks = self.generate_masks(vocals_weights, width, height)

        if vocals_audio is None or vocals_weights is None or vocals_masks is None or weights_graph is None:
            print("One or more outputs are invalid")
            return None, None, None, None

        return vocals_audio, vocals_weights.tolist(), vocals_masks, weights_graph
